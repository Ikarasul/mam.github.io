...
export function playGunScatterSound() {
  if (!useSound) return;
  const ctx = getAudioCtx();
  if (!ctx) return;
  ...
  // Retro sci-fi gun blaster / explosion sliding pitch sound
  const osc = ctx.createOscillator();
  const gain = ctx.createGain();
  ...
  // Fast retro click-cascade for scattered chips
  const osc2 = ctx.createOscillator();
  const gain2 = ctx.createGain();
  osc2.type = 'square';
  osc2.frequency.setValueAtTime(380, now + 0.12);
  osc2.frequency.setValueAtTime(620, now + 0.22);
  gain2.gain.setValueAtTime(0, now);
  gain2.gain.setValueAtTime(0.06, now + 0.12);
  gain2.gain.exponentialRampToValueAtTime(0.002, now + 0.35);
